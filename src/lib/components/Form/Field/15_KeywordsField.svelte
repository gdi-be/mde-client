    ...
    if (autoKeywords && containerElement) {
      containerElement.querySelectorAll('.mdc-chip').forEach((chip) => {
        ...
        if (textElement?.textContent && autoKeywords.includes(textElement?.textContent?.trim())) {
          ...
          chip.setAttribute('title', t('15_keywords.autogenerated'));
        }
      });
    }
  });
...
<div class="keywords-field" bind:this={containerElement}>
  <fieldset>
      ...
      {#snippet chip(chip)}
        <Chip class={autoKeywords.includes(chip) ? 'auto-keyword' : ''} {chip}>
          ...
          {#if !autoKeywords.includes(chip)}
            <TrailingIcon
              title={t('15_keywords.remove', { keyword: chip })}
    ...
    <Autocomplete
      ...
      search={searchItems}
      value={searchValue}
      label={t('15_keywords.autocomplete_label')}
    ...
    >
      ...
      {/snippet}
      {#snippet noMatches()}
        <Text>{t('15_keywords.no_matches')}</Text>
      {/snippet}
    </Autocomplete>
...
>
  <Title>{t('15_keywords.dialog_title')}</Title>
  <Content class="custom-keywords-content">
    <p>{t('15_keywords.dialog_content')}</p>
    <Textfield
      class="custom-keywords-input"
      textarea
      label={t('15_keywords.dialog_label')}
