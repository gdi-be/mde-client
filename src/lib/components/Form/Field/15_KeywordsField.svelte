    ...
    if (autoKeywords && containerElement) {
      containerElement.querySelectorAll('.mdc-chip').forEach((chip) => {
        ...
        if (textElement?.textContent && autoKeywords.includes(textElement?.textContent?.trim())) {
          ...
          chip.setAttribute('title', t('15_KeywordsField.autogenerated'));
        }
      });
    }
  });
...
<div class="keywords-field" bind:this={containerElement}>
  <fieldset class={[isInvalid && 'invalid']}>
      ...
      {#snippet chip(chip)}
        <Chip class={autoKeywords.includes(chip) ? 'auto-keyword' : ''} {chip}>
          ...
          {#if !autoKeywords.includes(chip)}
            <TrailingIcon
              title={t('15_KeywordsField.remove', { keyword: chip })}
    ...
    <Autocomplete
      ...
      search={searchItems}
      value={searchValue}
      label={t('15_KeywordsField.autocomplete_label')}
...
>
  <Title>{t('15_KeywordsField.dialog_title')}</Title>
  <Content class="custom-keywords-content">
    <p>{t('15_KeywordsField.dialog_content')}</p>
    <Textfield
      class="custom-keywords-input"
      textarea
      label={t('15_KeywordsField.dialog_label')}
      bind:value={newKeyword}
    />
  </Content>
  <Actions>
    <Button type="button" onclick={() => (dialogOpen = false)}>
      <Label>{t('general.cancel')}</Label>
    </Button>
    <Button variant="raised" onclick={addCustomKeywords} type="button">
      <Label>{t('general.add')}</Label>
    </Button>
  </Actions>
</Dialog>
